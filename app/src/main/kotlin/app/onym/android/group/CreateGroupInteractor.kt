...
import app.onym.android.chain.AnarchyCreateGroupPayload
import app.onym.android.chain.AnchorSelectionKey
import app.onym.android.chain.CanonicalFr
...
import app.onym.android.chain.OneOnOneCreateGroupPayload
...
open class CreateGroupInteractor(
    ...
    private suspend fun sendOffers(
        ...
        for ((index, inboxKey) in invitees.withIndex()) {
            // One fresh intro key per invitee, giving a clean 1:1
            // mapping from an inbound join request back to the person
            // the admin meant to invite. Deliberately `mint` and not
