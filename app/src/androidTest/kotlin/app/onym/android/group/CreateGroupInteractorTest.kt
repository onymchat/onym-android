...
import app.onym.android.support.ConfigurableContractTransport
...
class CreateGroupInteractorTest {
    ...
    fun setUp() {
        ...
        identityStore = IdentitySecretStore(
            ctx,
            prefsFileName = "app.onym.android.identity.create-group.${UUID.randomUUID()}",
        )
    ...
    fun create_withNoInvitees_savesGroupAndAnchorsOnChain() = runBlocking {
        ...
        assertEquals(app.onym.android.chain.SepGroupType.TYRANNY, group.groupType)
        assertEquals(app.onym.android.chain.SepTier.SMALL, group.tier)
    ...
    fun create_noActiveRelayer_throws() = runBlocking {
        ...
        val interactor = CreateGroupInteractor(
            ...
            contracts = contracts,
            groups = groups,
            networkPreference = app.onym.android.chain.StaticNetworkPreferenceProvider(
                app.onym.android.chain.AppNetwork.Testnet,
            ),
    ...
    fun create_mainnetSelected_withNoMainnetBinding_throwsNoContractBinding() = runBlocking {
        ...
        val mainnetPref = app.onym.android.chain.StaticNetworkPreferenceProvider(
            app.onym.android.chain.AppNetwork.Mainnet,
        )
        try {
    ...
    fun create_oneOnOne_zeroInvitees_throwsExactCountError() = runBlocking {
        ...
        try {
            interactor.create(
                name = "1v1",
                invitees = emptyList(),
                groupType = app.onym.android.chain.SepGroupType.ONE_ON_ONE,
            )
    ...
    fun create_oneOnOne_oneInvitee_anchorsViaOneOnOnePayload_andShipsInviteeBlsSecret() = runBlocking {
        ...
        assertEquals(app.onym.android.chain.SepGroupType.ONE_ON_ONE, group.groupType)
        assertTrue(group.isPublishedOnChain)
    ...
    fun create_anarchy_zeroInvitees_anchorsViaAnarchyPayload() = runBlocking {
        ...
        val group = interactor.create(
            name = "Anarchic group",
            invitees = emptyList(),
            groupType = app.onym.android.chain.SepGroupType.ANARCHY,
        )

        assertEquals(app.onym.android.chain.SepGroupType.ANARCHY, group.groupType)
        assertTrue(group.isPublishedOnChain)
    ...
    private fun makeInteractor(
        networkPreference: app.onym.android.chain.NetworkPreferenceProvider =
            app.onym.android.chain.StaticNetworkPreferenceProvider(
                app.onym.android.chain.AppNetwork.Testnet,
            ),
