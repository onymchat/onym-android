...
class IdentitySecretStore(
    ...
    internal fun listSnapshots(): List<Pair<IdentityId, StoredSnapshot>> =
    ...
    internal fun loadCurrent(): IdentityId? {
        val raw = try {
    ...
    internal fun saveCurrent(id: IdentityId?) {
     ...
     *  Internal — returns secret material; the only consumers are
     *  [IdentityRepository] and this module's tests. */
    internal fun load(id: IdentityId): StoredSnapshot? {
        val raw = try {
    ...
    internal fun save(id: IdentityId, snapshot: StoredSnapshot) {
        val raw = try {
    ...
    internal fun wipe(id: IdentityId) {
        val ids = listIds()
        if (id !in ids) return
        val newIds = ids - id
        val indexRaw = json.encodeToString(
    ...
    companion object {
        internal const val DEFAULT_PREFS_FILE_NAME = "app.onym.android.identity"
